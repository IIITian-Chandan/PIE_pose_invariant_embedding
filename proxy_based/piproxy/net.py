...
parser.add_argument('--lr', '--learning-rate', action='store', default=0.00001, type=float,
                    help='learning rate (default: 0.00001)')
parser.add_argument('--m', '--momentum', action='store', default=0.9, type=float, help='momentum (default: 0.9)')
parser.add_argument('--w', '--weight-decay', action='store', default=0, type=float,
                    help='regularization weight decay (default: 0.0)')
parser.add_argument('--train_f', action='store_false', default=True, help='Flag to train (STORE_FALSE)(default: True)')
parser.add_argument('--useGPU_f', action='store_false', default=True,
                    help='Flag to use GPU (STORE_FALSE)(default: True)')
parser.add_argument('--preTrained_f', action='store_false', default=True,
                    help='Flag to pretrained model (default: True)')
parser.add_argument('--gpu_num', '--list', type=int, nargs='+', help='gpu_num', required=True)
parser.add_argument("--net", default='VGG_avg', const='VGG_avg', nargs='?', choices=['VGG_avg'],
                    help="net model(default:VGG_avg)")
parser.add_argument("--dataset", default='ObjectPI', const='ObjectPI', nargs='?', choices=['ModelNet', 'ObjectPI'],
                    help="Dataset (default:ObjectPI)")
...
def main():
    ...
    ch = logging.FileHandler(
        'log/logfile_' + arg.net + '_' + arg.dataset + '_' + str(arg.alpha) + '_' + str(arg.beta) + '_' + str(
            arg.trial) + '.log')
    ...
    # for cpu mode
        ...
        model
    model_path = 'model/model_' + arg.net + '_' + arg.dataset + '_' + str(arg.alpha) + '_' + str(arg.beta) + '_' + str(
        arg.trial) + '.pt'
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (batch_x, batch_y) in enumerate(dataloader_trn):
                        ...
                        if not o == y[v]:
                            ...
                            if denominator is None:
                                ...
                                denominator = denominator + torch.exp(
                                    (arg.alpha * neg_dist_v_s + arg.beta * neg_dist_s_c))
            ...
            if batch_idx % 10 == 0:
                ...
                print('==>>> epoch:{}, batch index: {}, loss:{}, accuracy:{}'.format(epoch, batch_idx,
                                                                                     loss.cpu().detach().numpy(),
                                                                                     accuracy))
                logger.info('==>>> epoch:{}, batch index: {}, loss:{}, accuracy:{}'.format(epoch, batch_idx,
                                                                                           loss.cpu().detach().numpy(),
                                                                                           accuracy))
